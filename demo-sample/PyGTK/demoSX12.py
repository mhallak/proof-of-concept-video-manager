...
# Revision:
#
#    April,  2012	  Pete Eberlein	   Initial, designed for Sensoray Model 2253
#    August, 2012	  Charlie X. Liu   Added support for Sensoray Model 812 and SX11
#
# Notes:
# 
#    To use this PyGTK demo program, you need both MPlayer and MEncoder tools.
#    Therefore, please install both programs, to run video Preview and A/V 
#    Capturing & Recording properly. 
#    For example, to the customers with Ubuntu LTS distros, simply run followings:
#
#         sudo apt-get install mplayer 
#         sudo apt-get install mencoder 
#
#    to get the MPlayer and MEncoder installed.
#
...
def alsa_devices(driver):
	asound = ctypes.CDLL("libasound.so.2")
	asound.snd_ctl_card_info_malloc.argtypes = [ctypes.POINTER(ctypes.POINTER(ctypes.c_ubyte))]
	asound.snd_ctl_card_info_get_driver.restype = ctypes.c_char_p

	hw_info = ctypes.POINTER(ctypes.c_ubyte)()
	ctl = ctypes.POINTER(ctypes.c_ubyte)()
	asound.snd_ctl_card_info_malloc(ctypes.byref(hw_info))
	...
	driver = driver.lower()
	while True:
		hw_name = ctypes.create_string_buffer("hw:%d" % i)
		if asound.snd_ctl_open(ctypes.byref(ctl), hw_name, 0) != 0:
			break
		asound.snd_ctl_card_info(ctl, hw_info)
		if driver == asound.snd_ctl_card_info_get_driver(hw_info).lower() or \
			(driver == "tw6869" and asound.snd_ctl_card_info_get_driver(hw_info) == "TW6869 audio"):
			lst.append(hw_name.value)
		i += 1
	asound.snd_ctl_close(ctl)
	asound.snd_ctl_card_info_free(hw_info)
	return lst

def determine_board_num(dev, driver):
	d = int(string.replace(dev, "/dev/video", ""))
	cp = v4l2_capability()
	brd = 0
	while d > 0:
		d -= 1
		try:
			fd = open("/dev/video%d" % d, "rw")
			ioctl(fd, VIDIOC_QUERYCAP, cp)
			if cp.driver == driver:
				brd += 1
		except:
			pass
...
#		if self.demo.audio_hw != None: cmd += "forceaudio:alsa:adevice=%s:amode=1:audiorate=32000:immediatemode=0:" % string.replace(self.demo.audio_hw, ":", ".")
		if cmd != "": cmd = " -tv " + cmd
		if debug: cmd += " -quiet"
		...
		dprint(cmd)
		try: 
			self.p = subprocess.Popen(cmd, stdin=subprocess.PIPE, shell=True)
			try: self.p.wait()
			except: pass
		except:
			self.demo.message_box("Could not open mplayer")
...
class CaptureThread(threading.Thread):
	...
	def run(self):
		...
		if self.sock:
			...
			except: status = "Could not open capture program - check sdk installation"
...
class CaptureThread_sx11or12(threading.Thread):
	def __init__(self, devname, audio_hw, outname, outfmt, videnc, width, height, norm, input, audioSampleRate, demo):
		threading.Thread.__init__(self)
		self.devname = devname
		self.adevice = audio_hw
		self.outname = outname
		self.outfmt = outfmt
		self.videnc = videnc
		self.width = width
		...
		self.action = 'Capturing & Recording'
		self.quit = False
		self.demo = demo
        def run(self):
                cmd = [ "mencoder", 
                        "tv://",
                        "-tv",
                        #"driver=v4l2:device=%s:norm=ntsc:input=1:width=%d:height=%d:buffersize=512:audiorate=44100:amode=1:alsa" % (self.devname,self.width,self.height),
                        "driver=v4l2:device=%s:norm=ntsc:width=%d:height=%d:forceaudio:audiorate=%d:alsa:adevice=hw.%s:amode=0" % (self.devname,self.width,self.height,self.audioSampleRate,self.adevice[3:6]),
                        #"-noskip",
                        #"-ovc", "copy",
                        "-ovc", "raw",
                        #"-oac", "copy",
                ...
                try:
                	...
                	if self.process.returncode > 0:
	                        status = "Status: Capture Stopped"
        	except: status = "Could not open mencoder"
                self.demo.update_status(self.devname, status)
        def stop(self):
                try: self.process.terminate()
                except: pass
...
class Demo:
	...
	def set_audio_hw(self, widget, data=None):
		#print "data =", data
		self.audio_hw = data
		self.audio_hw += ',0'
		#print "self.audio_hw =",self.audio_hw	#concern [3:8]

	def set_audio_sampling_rate(self, widget, data=None):
		#print "data =", data
		self.audioSampleRate = data
		#print "self.audioSampleRate =",self.audioSampleRate
	...
	def launch_capture_sx11or12(self, widget):
		if self.is_device_busy():
			return
		if self.outfmt != V4L2_PIX_FMT_YUYV and self.outfmt != V4L2_PIX_FMT_UYVY:
			if not self.message_box_yes_no("The format %s is not recommended for recording. Recording anyway?" % (self.outfmt_name)):
				return
		name = "output"
		videnc = self.get_videnc()
		if self.outfmt == V4L2_PIX_FMT_YUYV:
			name += ".avi"
		elif self.outfmt == V4L2_PIX_FMT_UYVY:
			name += ".avi"
		else:
		...
		print "hw =", self.audio_hw

		thread = CaptureThread_sx11or12(self.vd.name, self.audio_hw, save.get_filename(), self.outfmt, videnc, self.width, self.height, self.norm, self.input, self.audioSampleRate, self)
		save.destroy()
		self.threads.append(thread)
		thread.start()
		self.status.set_text("Status: Capturing & Recording")
		return		
	...
	def __init__(self):
		...
		self.window.set_title("SX12 V4L2 Capture Demo")
		...
		if self.vd == None:
			...
			if cp.capabilities & V4L2_CAP_VIDEO_CAPTURE:
				...
				for dim in [
					...
					if self.width == dim[0] and self.height == dim[0]:
						menudefault = i
					i += 1
			...
			while True:
				...
				if queryctrl.type == V4L2_CTRL_TYPE_BOOLEAN:
					...
					entry.set_text(default_value.value)
			...
			if cp.capabilities & V4L2_CAP_VIDEO_CAPTURE:
				...
				if audio != None:
					...
					hbox.show()
			if audio == None:
				# no driver audio support, list alsa with same driver names
				hbox = gtk.HBox(False, 10)
				label = gtk.Label("ALSA Device:")
				hbox.pack_start(label, False, False, 0)
				label.show()
				...
				opt = gtk.OptionMenu()
				menu = gtk.Menu()
				for hw in alsa_devices(cp.driver):
					menu.append(make_menu_item(hw, self.set_audio_hw, hw))
					audio_hw.append(hw)
					#print "audio_hw =",audio_hw
				opt.set_menu(menu)
				hbox.pack_start(opt, False, False, 0)
				opt.set_history(determine_board_num(self.vd.name, cp.driver))
				opt.show()
				...
				if len(audio_hw) > 0:
					self.audio_hw = audio_hw[0]
					self.audio_hw += ',0'
					#print "self.audio_hw =",self.audio_hw	#concern [3:8]

				# select Audio Sampling Rate
				hbox = gtk.HBox(False, 10)
				label = gtk.Label("Audio Sampling Rate:")
				hbox.pack_start(label, False, False, 0)
				label.show()
				...
				menudefault = 0
				i = 0
				self.audioSampleRate = 32000	# as default
				for dim in [
					[8000, "8KHz"],
					[16000, "16KHz"],
					[32000, "32KHz"],
					...
					menu.append(make_menu_item("%s" % (dim[1]), self.set_audio_sampling_rate, dim[0]))
					if self.audioSampleRate == dim[0]:
						menudefault = i
					i += 1
			...
			if cp.capabilities & V4L2_CAP_VIDEO_CAPTURE:
				...
				capture = gtk.Button("Capture & Record")
				#capture.connect_object("clicked", self.launch_capture, None)	//for 2253
				capture.connect_object("clicked", self.launch_capture_sx11or12, None)
				capture.show()
