...
#if 0
static void reg_printk(struct tw686x_dev *dev, int reg)
{
	unsigned int val = 0;
	val = reg_read(dev, (unsigned int) reg);
	printk("REGISTER %x, val: %x\n", reg, val);
	return;
}

static void dump_registers(struct tw686x_dev *dev)
{
	int i;
	reg_printk(dev, AUDIO_CONTROL2);
	reg_printk(dev, AUDIO_CONTROL1);

	reg_printk(dev, DMA_CMD);
	reg_printk(dev, DMA_CHANNEL_ENABLE);
	reg_printk(dev, AUDIO_CONTROL1);
	reg_printk(dev, SYS_SOFT_RST);
	reg_printk(dev, SRST[0]);
	reg_printk(dev, SRST[1]);
	reg_printk(dev, DMA_CONFIG);
	reg_printk(dev, DMA_CHANNEL_TIMEOUT);
	reg_printk(dev, DMA_TIMER_INTERVAL);
	for (i = 0; i < 8; i++) {
		reg_printk(dev, VIDEO_FIELD_CTRL[i]);
		reg_printk(dev, VDMA_CHANNEL_CONFIG[i]);
		reg_printk(dev, VDMA_WHP[i]);
		reg_printk(dev, SDT[i]);
		reg_printk(dev, SDT[i]);
		reg_printk(dev, SDT_EN[i]);
		reg_printk(dev, VDMA_CHANNEL_CONFIG[i]);
		reg_printk(dev, VDELAY_LO[i]);
		reg_printk(dev, HACTIVE_LO[i]);
		reg_printk(dev, VIDEO_SIZE[i]);
		reg_printk(dev, ADMA_P_ADDR[i]);
		reg_printk(dev, ADMA_B_ADDR[i]);
	}
	reg_printk(dev, VIDEO_CONTROL1);
	reg_printk(dev, PHASE_REF);
	reg_printk(dev, MISC2[0]);
	reg_printk(dev, VCTRL1[0]);
	reg_printk(dev, LOOP[0]);
	reg_printk(dev, VCTRL1[1]);
	reg_printk(dev, LOOP[1]);
	reg_printk(dev, MISC2[1]);
	return;
}
#endif
