/*
 * Sensoray TW6869 audio port
 * Copyright 2018 Sensoray 
 * Based on audio support in Linux kernel driver
 * but backward compatible with current VBUF2 driver.
 * seperates audio channels into separate devices for easier
 * VLC usage
 *
...
void TW68_disable_channel(struct TW68_dev *dev, unsigned int channel)
{
	...
	if (!dma_en)
		dma_cmd = 0;
	reg_writel(DMA_CHANNEL_ENABLE, dma_en);
	reg_writel(DMA_CMD, dma_cmd);
				  ...
				  struct TW68_audio_channel *ac)
{
	...
	for (pb = 0; pb < 2; pb++) {
		...
		reg_writel(reg, ac->dma_descs[pb].phys);
	}
	return 0;
}
...
void TW68_audio_free(struct TW68_dev *dev)
{
	...
	reg_writel(DMA_CMD, dma_cmd & ~0xff00);
	reg_writel(DMA_CHANNEL_ENABLE, dma_ch_mask & ~0xff00);
	spin_unlock_irqrestore(&dev->slock, flags);
...
int TW68_audio_init(struct TW68_dev *dev)
{
	...
	reg_writel(AUDIO_CONTROL1, BIT(0));
